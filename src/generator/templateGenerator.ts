...
function generateSummary(overview: TrafficOverview, anomalies: AnomalyCandidate[], weather: WeatherResult): string {
  ...
  if (weather.status === 'Storm') {
    return `Unique Users changed ${usersPct} in the last 24h. ${drops.length} significant drop${drops.length !== 1 ? 's' : ''} detected across dimensions.`
  }
  if (drops.length > 0 && spikes.length > 0) {
    return `Unique Users changed ${usersPct}. Mixed signals: ${drops.length} drop${drops.length !== 1 ? 's' : ''} and ${spikes.length} rise${spikes.length !== 1 ? 's' : ''} worth noting.`
  }
  if (drops.length > 0) {
    return `Unique Users changed ${usersPct} with ${drops.length} notable decline${drops.length !== 1 ? 's' : ''}.`
  }
  if (spikes.length > 0) {
    return `Unique Users changed ${usersPct} with ${spikes.length} growth signal${spikes.length !== 1 ? 's' : ''}.`
  }
  return `Unique Users changed ${usersPct}. No significant anomalies detected.`
}
...
export const templateGenerator: BriefGenerator = {
  generateBrief(
    ...
    return {
      ...
      summary: generateSummary(overview, anomalies, weather),
